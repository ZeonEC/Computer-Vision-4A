...
std::vector<std::vector<cv::Point2f>> get_grid_points(int nb_calib, int cur_cam) {
	...
	cv::Size pattern_size(9, 7); // nombre de coins interieurs dans la grille (7x9)


	for (int i = 1; i < nb_calib + 1; i++)
	{
		//    ../../../calibration_images/calib1.jpg
		std::string FILENAME = "../../../calibration_images/calib_cam" + std::to_string(cur_cam + 1) + "_" + std::to_string(i) + ".png";
